---
title: 'Extintor PQS FR-10'
description: 'Extintor de Polvo Químico Seco 10 lbs – Certificado INEN 731'
main:
  id: 2
  content: |
    El Extintor PQS FR-10 de FULLREX es tu primera línea de defensa contra incendios Clase A, B y C. Fabricado con acero de alta presión y cargado con polvo químico seco de máxima calidad, está certificado bajo la norma INEN 731 para cumplir con las exigencias de Bomberos en empresas y vehículos de carga en Ecuador.
  imgCard: '@/images/product-image-2.avif'
  imgMain: '@/images/product-image-main-2.avif'
  imgAlt: 'Extintor de polvo químico seco 10 lbs FULLREX certificado INEN'
tabs:
  - id: 'tabs-with-card-item-1'
    dataTab: '#tabs-with-card-1'
    title: 'Descripción'
  - id: 'tabs-with-card-item-2'
    dataTab: '#tabs-with-card-2'
    title: 'Especificaciones'
  - id: 'tabs-with-card-item-3'
    dataTab: '#tabs-with-card-3'
    title: 'Planos Técnicos'
longDescription:
  title: 'Protección Certificada contra Incendios'
  subTitle: |
    El FR-10 está diseñado para empresas, locales comerciales y vehículos de transporte. Cumple estrictamente con la norma INEN 731 y las disposiciones del Cuerpo de Bomberos del Ecuador, garantizando tu tranquilidad y la seguridad de tu patrimonio ante cualquier emergencia.
  btnTitle: 'Solicitar recarga o cotización por WhatsApp'
  btnURL: 'https://wa.me/593961661643?text=Hola%20FULLREX%2C%20me%20interesa%20el%20Extintor%20PQS%20FR-10%20de%2010%20lbs.%20%C2%BFPueden%20darme%20m%C3%A1s%20informaci%C3%B3n%3F'
descriptionList:
  - title: 'Certificación INEN 731'
    subTitle: 'Cumple con la normativa ecuatoriana INEN 731 y es aceptado por el Cuerpo de Bomberos para auditorías técnicas.'
  - title: 'Triple Acción A, B, C'
    subTitle: 'Efectivo contra fuegos de materiales sólidos (A), líquidos inflamables (B) y equipos eléctricos (C).'
  - title: 'Servicio de Recarga'
    subTitle: 'FULLREX ofrece servicio de recarga, mantenimiento y certificación anual a domicilio o en nuestro local en Quitumbe.'
specificationsLeft:
  - title: 'Capacidad'
    subTitle: '10 lbs (4.5 kg) de polvo químico seco ABC de alta eficiencia.'
  - title: 'Presión de Trabajo'
    subTitle: '195 PSI (13.4 bar) con válvula de seguridad incluida para operación segura.'
  - title: 'Alcance de Descarga'
    subTitle: 'Hasta 5 metros de distancia con tiempo de descarga de 12 a 15 segundos.'
  - title: 'Material del Cilindro'
    subTitle: 'Acero de alta resistencia con recubrimiento anticorrosivo, apto para interiores y exteriores.'
specificationsRight:
  - title: 'Temperatura de Operación'
    subTitle: 'Funciona entre -30°C y +60°C. Ideal para Ecuador en cualquier altitud o clima.'
  - title: 'Clases de Fuego'
    subTitle: 'Apto para fuegos Clase A (sólidos), Clase B (líquidos) y Clase C (eléctricos).'
  - title: 'Certificaciones'
    subTitle: 'Certificado INEN 731 e ISO 11601. Incluye manguera flexible, pasador de seguridad y sello de garantía.'
  - title: 'Aplicaciones Recomendadas'
    subTitle: 'Locales comerciales, oficinas, bodegas, vehículos de carga pesada, industrias y viviendas.'
blueprints:
  first: '@/images/blueprint-3.avif'
  second: '@/images/blueprint-4.avif'
---
